...
const useAudioPlayer = (songs) => {
  // 1. Define State and Refs FIRST
  const [audioState, dispatch] = useReducer(audioReducer, initialAudioState);
  const [duration, setDuration] = useState(0);
  const audioRef = useRef(null); // Must be null for <audio>
  const previousVolumeRef = useRef(1); // Required for mute/unmute logic

  // 2. Sync Effect (Updates index when playlist changes)
  useEffect(() => {
    if (audioState.currentSong && songs.length > 0) {
      const newIndex = songs.findIndex((s) => s.id === audioState.currentSong.id);
      if (newIndex !== -1 && newIndex !== audioState.currentIndex) {
        dispatch({
          type: "SET_CURRENT_TRACK",
          payload: { index: newIndex, song: audioState.currentSong },
        });
      }
    }
  }, [songs]);
